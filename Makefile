...
benchmarks:
	PYTHON=$(PYTHON) MPLCONFIGDIR=$(MPLCONFIGDIR) JSON_LOG_ROOT=$(JSON_LOG_ROOT) EMBEDDING_PNG_ROOT=$(EMBEDDING_PNG_ROOT) REPEATS=$(REPEATS) TOPOLOGY_REPEATS=$(TOPOLOGY_REPEATS) MAX_POINTS=$(MAX_POINTS) FULL_DATASETS=$(FULL_DATASETS) UMAP_MAX_POINTS=$(UMAP_MAX_POINTS) METRIC_SUBSAMPLE=$(METRIC_SUBSAMPLE) TOPOLOGY_SAMPLE_FRACTION=$(TOPOLOGY_SAMPLE_FRACTION) TOPOLOGY_STEPS=$(TOPOLOGY_STEPS) scripts/run_benchmarks.sh $(OUTPUT_ROOT)
